<template>
  <el-upload ref="uploadAvatarRef" v-model:file-list="uploadFileList"
    :class="[showUpload && !preview ? '' : 'hide-upload']" action="#" list-type="picture-card" :auto-upload="false"
    :multiple="multiple" :limit="limit" :on-change="uploadChange">
    <el-icon>
      <Plus />
    </el-icon>
    <template #file="{ file }">
  ...
  <el-image-viewer v-if="dialogVisible" :url-list="uploadFileList.map(v => v.url)" :initial-index="previewIndex"
...
const uploadChange = () => {
  nextTick(() => {
    ...
    // change the parent's file value
